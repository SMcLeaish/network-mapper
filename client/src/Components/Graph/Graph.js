...
import { Box, Select, MenuItem, FormControl, InputLabel, Popover, CircularProgress } from '@mui/material';
...
function Graph({ name }) {
	...
	const [colorMetric, setColorMetric] = useState('community');
	...
	if (loading) {
		...
		return (
			<div style={{ position: 'relative', height: '100%', width: '100%' }}>
				...
				<FormControl style={{ position: 'absolute', top: '10px', left: '260px', zIndex: 1, width: '120px' }}>
					<InputLabel id="color-metric-label" shrink>Color Metric</InputLabel>
					<Select
						labelId="color-metric-label"
						value={colorMetric}
						onChange={(event) => setColorMetric(event.target.value)}
						style={{ height: '30px' }}
					>
					...
					stylesheet={[
						{
							...
							style: {
								...
								'background-color': (node) => getColorForNode(node.data(colorMetric)),
								'color': '#F8F4E3',
								'text-valign': 'bottom',
								'text-halign': 'center',
